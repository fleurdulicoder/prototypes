...
var Banner = function(bannerId, bannerSlides, autoRotateFlag, animateFlag) {
  var doRotation = autoRotateFlag || false, rotation;
  var element = document.getElementById(bannerId) || null;
  var slides = bannerSlides || [];
  var doAnimate = animateFlag || false;
  if (!element || slides.length == 0) return;

  // if animate in view, wrap each element into span
  // add timeline on load
  ...
  function animateInView() {
    ...
    timeline.staggerFrom('.float-up', 1, {
        y: "500%",
        opacity: 0,
        ease: Power4.easeOut
      }, 0.15);
  ...
  function setContent(index, slide) {
    if (index === 0) {
      slide.number.innerHTML = (index < 10)
        ? '<span class="float-up">0'+(index+1)+'</span>'
        : '<span class="float-up">'+(index+1)+'</span>';
      slide.title.innerHTML = '<span class="float-up">'+slides[index].title+'</span>';
      slide.text.innerHTML = '<span  class="float-up">'+slides[index].text+'</span>';
    } else {
